...
### Soft Actor-Critic (SAC) Learning Algorithm

The addition of the Soft Actor-Critic (SAC) learning algorithm is a significant improvement to the power of our AI engine. It is not set as the default algorithm yet, so to start using it pass the `--learning-algorithm sac` parameter to `spice train`. We'd love to get your feedback on how its working!
